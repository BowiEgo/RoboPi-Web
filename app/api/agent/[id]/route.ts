...
export async function POST(
  ...
  try {
    ...
    const existing = ctx.sessions.getRpcSession(id);
    if (body.type === "set_tools") {
      ...
      const changed = await ctx.sessions.setRpcSessionTools(id, filePath, toolNames);
    ...
    const { session } = await ctx.sessions.startRpcSession(id, filePath, undefined, {
...
export async function GET(
  ...
  try {
    const ctx = await getRoot();
    const session = ctx.sessions.getRpcSession(id);
